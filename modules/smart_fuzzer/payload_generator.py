...
class PayloadGenerator:
    ...
    def _call_ai(
        ...
        """
        Unified AI call through the router with telemetry logging.

        Raises:
            NoAIAvailableError: if router is None or router raises its own equivalent.
        """
        if not self._router:
        ...
        start_ts = time.monotonic()
        try:
            # Map string complexity to enum if needed
            complexity_enum = complexity
            if _AI_ROUTER_AVAILABLE and isinstance(complexity, str):
            ...
            if isinstance(result, dict):
                ...
                logger.debug(
                    "AI call completed in %.2fs | tokens: %s | prompt_len: %d",
                    elapsed,
                    tokens,
                    len(prompt),
                )
                ...
                text = str(result)
                logger.debug(
                    "AI call completed in %.2fs | prompt_len: %d", elapsed, len(prompt)
                )
            ...
            # Check if this is the router's own "no AI" error
