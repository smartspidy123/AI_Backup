...
class SmartFuzzer:
    ...
    def _process_task_wrapper(self, task_json: str):
        ...
        try:
            ...
            # Update status
            self._set_status(task_id, "processing")

            result = self._process_task(task_dict)

            # Push result
            self._push_result(result)
            self._set_status(task_id, result.get("status", "completed"))
    ...
    def _process_task(self, task_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = {
            ...
            "stats": stats,
            "errors": errors[:50],  # Cap errors list
            "started_at": started_at,
    ...
    def _set_status(self, task_id: str, status: str):
        """Update task status in Redis. Fails silently if Redis is down."""
        try:
            if self._redis:
                key = f"{self._status_prefix}{task_id}"
                self._redis.set(key, status, ex=3600)  # 1-hour TTL
        except Exception as e:
            logger.debug("Failed to set status for task '%s': %s", task_id, e)
